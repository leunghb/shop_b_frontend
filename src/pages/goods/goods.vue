<template>
    <div class="goods">
        <el-select class="goodsCategory" v-model="currentGoodsCategory" placeholder="商品分类" @change="filtrate">
            <el-option
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <span style="margin-left: 10px">{{ scope.row.addedTime }}</span>
                </template>
            </el-table-column>
            <el-table-column
                    label="商品名称"
                    width="180">
                <template slot-scope="scope">
                    <span class="beyondOmission">{{ scope.row.mainTitle }}</span>
                </template>
            </el-table-column>
            <el-table-column
                    label="商品封面"
            ...
            <el-table-column label="操作" fixed="right" width="150">
                <template slot-scope="scope">
                    ...
                    <el-button
                            size="mini"
                            type="danger"
                            @click="goodsDel(scope.$index, scope.row)">删除
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    ...
    import {formatDate, formatMoney, hexadecimalColors, padNumber} from "../../utils/common";
    ...
    export default {
        ...
        methods: {
            ...
            getGoodsList() {
                ...
                post(api.getGoodsList, params).then(res => {
                    let data = res.data;
                    if (data.code == 0) {
            ...
            search() {
                if (this.searchText == null || this.searchText.length == 0) return false;
                this.page = 1;
                this.getGoodsList();
            },
            filtrate() {
